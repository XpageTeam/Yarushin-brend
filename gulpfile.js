...
const remotePathCss = templatePath+"css",
	...
	remotePathImg = templatePath+"images";
...
gulp.task('imagemin', () =>  
	gulp.src('src/images/**/*', {since: gulp.lastRun("imagemin")})
		 .pipe($.cache($.imagemin([
			$.imagemin.gifsicle({
				interlaced: true,
			}),
...
watch = _ => {
	...
	gulp.watch("dist/images/**/*", gulp.series("deploy:img"))
};

gulp.task("deploy-to-server", gulp.series(gulp.parallel("postcss", "imagemin"), gulp.parallel(local, watch)));

gulp.task("finish:him", gulp.series(gulp.parallel("postcss", "imagemin"), gulp.parallel("deploy:css", "deploy:js")));

gulp.task("default", gulp.series(gulp.parallel("postcss", "imagemin", "move:fonts"), gulp.parallel(local, "browser-sync")))
